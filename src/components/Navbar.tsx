    ...
    >
      <div className="container-custom flex justify-between items-center">
        <a href="#home" className="flex items-center">
          <h1 className={`text-2xl font-bold ${isScrolled ? 'text-gradient' : 'text-white'}`}>ORBIT</h1>
        </a>
      ...
      {/* Mobile Menu */}
      {isMenuOpen && (
        <div className="md:hidden fixed inset-0 top-16 z-40 bg-white/95 backdrop-blur-md p-6">
          <nav className="flex flex-col items-center gap-6 pt-8">
            {navLinks.map((link) => (
              <a 
                ...
                className="text-xl font-medium text-charcoal hover:text-purple transition-colors"
              >
                {link.name}
              </a>
            ))}
